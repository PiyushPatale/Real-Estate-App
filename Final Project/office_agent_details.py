...
bg = PhotoImage(file = "BG_agent_details_1.png")
label1 = Label(root10, image = bg)
label1.place(x=0, y=0)

# Connect to Database
try:
    conn = mysql.connect(host = 'localhost', user = 'root', password = 'Piyush@2003')
...
aid = Label(root10, text = row[0], fg = "white", bg = "#292F33", font = ("yu gothic ui", 20, 'bold')).place(x=275, y=140)
name = Label(root10, text = row[2], fg = "white", bg = "#292F33", font = ("yu gothic ui", 20, 'bold')).place(x=275, y=185)
mail = Label(root10, text = row[3], fg = "white", bg = "#292F33", font = ("yu gothic ui", 20, 'bold')).place(x=275, y=230)
rate = Label(root10, text = row[4], fg = "white", bg = "#292F33", font = ("yu gothic ui", 20, 'bold')).place(x=320, y=271)
...
def ava_com():
    ...
    bg = PhotoImage(file = "BG_matched_office_1.png")
...
def sol_com():
    ...
    bg = PhotoImage(file = "BG_office_agent_sold_1.png")
    ...
    #  rent COUNT     
    query = 'select count(p_id) from Property where a_id = %s and status = "R"' # COUNT
    mycursor.execute(query, [sys.argv[1]])
    ans = mycursor.fetchone()
    
    num = Label(root12, text = ans[0], width = 21, fg = 'white', border = 0, bg = '#292F33', font = ('Microsoft YaHei UI Light', 15))
    num.place(x = 658, y = 515)
    ...
    buttonS = Button(root12, text = 'SUBMIT', width = 8, background= '#E5E5E5', foreground= 'black', activebackground = '#E5E5E5', activeforeground = 'grey', font = ('yu gothic ui', 13, 'bold'), border = 0, command = topOB).place(x = 920, y = 558)
...
ava_button = Button(root10, text = "VIEW\nAVAILABLE\nPROPERTIES", fg = "white", bg = "black", activeforeground = "grey", activebackground = "black", font = ("yu gothic ui", 12), width = 11, border = 0, command = ava_com).place(x = 370, y = 587)
sold_button = Button(root10, text = "VIEW\nSOLD/RENTED\nPROPERTIES", fg = "white", bg = "black", activeforeground = "grey", activebackground = "black", font = ("yu gothic ui", 12), width = 11, border = 0, command = sol_com).place(x = 660, y = 587)
